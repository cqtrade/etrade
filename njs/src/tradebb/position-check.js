...
const handlePosition = async (pos) => {
	try {
		if (pos.size > 0) {
			...
			const currPNL = calculatePnlPercentage(
				pos.side,
				entryPrice,
...
const flow = async () => {
	try {
		...
		console.log('ERROR trading flow request failed: ' + error.message);
...
function engine() {
	...
	setTimeout(() => {
		flow()
			...
			.catch((error) => {
				console.log('positions engine flow failed: ' + error.message);
				logger.error('positions engine flow failed: ' + error.message);
				engine();
			});
	}, interval);
}
...
// flow().catch(error => {
//   console.log('positions flow failed: ' + error.message);
// });
